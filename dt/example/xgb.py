...
#  ########################lightGBM##########################
param_lgbm = {'task': 'train',
              'boosting_type': 'gbdt',  # 设置提升类型
              'objective': 'binary',  # regression 目标函数  regressiong默认regressiong_12
              'metric': 'auc',  # rmse 评估函数
              'max_depth': 6,  # 树的深度  按层
              'num_leaves': 50,  # 由于leaves_wise生产，小于2^max_depth  按leaf_wise
              'learning_rate': 0.05,
              'subsample': 0.8,  # 数据采样
...
num_round = 100
start = datetime.now()
lgbm = lgb.train(param_lgbm, train_data, num_round, valid_sets=[train_data, val_data], early_stopping_rounds=50)
stop = datetime.now()
# lightgbm
