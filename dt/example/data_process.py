...
priors = pd.read_csv(idir + 'order_products_prior.csv', dtype={
    ...
    'reordered': np.int8})
train = pd.read_csv(idir + 'order_products_train.csv', dtype={
    'order_id': np.int32,
    'product_id': np.int16,
    'add_to_cart_order': np.int16,
    'reoredered': np.int8})
...
print('product {}:{}'.format(products.shape, ','.join(products.columns)))
...
def features(selected_orders, labels_given=False):
    ...
    labels = []
    i = 0
    for row1 in selected_orders.itertuples():
        i += 1
        if i % 10000 == 0:
            print('order row', i)
