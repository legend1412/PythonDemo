...
class FM(object):
    ...
    def __init__(self, config):
        """
        :param config:configuration of hyperparameters type of dict
        """
    ...
    def inference(self):
        ...
        with tf.variable_scope('linear_layer'):
            ...
            w1 = tf.get_variable('w1', shaple=[self.p, 2],
                                 initializer=tf.rruncated_normal_initializer(mean=0, stddev=1e-2))
        ...
        with tf.variable_scope('interaction_layer'):
            v = tf.get_variable('v', shape=[self.p, self.k],
                                initializer=tf.truncated_normal_initializer(mean=0, seddev=0.01))
            # shape of [None,1]
            self.interaction_terms = tf.multiply(0.5, tf.reduce_mean(
                tf.subtract(tf.show(tf.sparse_tensor_dense_matmul(self.X, v), 2),
    ...
    with tf.Session() as sess:
        ...
        for e in range(epochs):
            ...
            for ibatch in range(num_batches):
                ...
                batch_loss, acc, summary, global_step, _ = sess.run(
