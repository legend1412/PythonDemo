...
print(tf.argmax(a, axis=0))  # tf.Tensor([1 0 0 0 0],shape=(6,),dtype=int64)

a = tf.truncated_normal([3, 3], stddev=0.1)
y_ = [[0, 0, 1], [0, 1, 0]]
y_cov = [[0.2, 0.1, 0.7], [0, 0.8, 0.2]]

with tf.Session() as sess:
    print(sess.run(tf.argmax(y_, 1)))  # [2,1]
    print(sess.run(tf.argmax(y_cov, 1)))  # [2,1]
    print(sess.run(tf.equal(tf.argmax(y_, 1), tf.argmax(y_cov, 1))))  # [True,True]
    print(sess.run(tf.cast(tf.equal(tf.argmax(y_, 1), tf.argmax(y_cov, 1)), tf.float32)))  # [1.1.]
valid_size = 16
# 验证单词从频率最高的100个单词中抽取
valid_window = 100
# 随机抽取验证样板集
...
x1 = tf.Variable([0, 0, 0, 1, 1, 0])
x_feat_ind = tf.Variable([4, 3])
