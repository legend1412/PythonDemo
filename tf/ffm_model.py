...
class FFM(object):
    ...
    def inference(self):
        ...
        with tf.variable_scope('linear_layer'):
            ...
            w1 = tf.get_variable('w1', shape=[self.p, 2], initializer=tf.truncated_normal_initializer(mean=0, stddev=0.2))
        ...
        with tf.variable_scope('field_aware_interaction_layer'):
            ...
            # build dict tofind f,key of feature,value of field
            for i in range(self.p):
                for j in range(i + 1, self.p):
                    self.field_aware_interaction_terms += tf.multiply(
                        tf.reduce_sum(tf.multiply(v[i, self.feature2field[i]], v[j, self.feature2field[j]])), tf.multiply
                        (self.X[:i], self.X[:, j]))
    ...
    def add_loss(self):
        ...
        mear_loss = tf.reduce_mean(cross_entropy)
        self.loss = mear_loss
