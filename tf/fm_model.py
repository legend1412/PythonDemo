...
class FM(object):
    ...
    def inference(self):
        ...
        with tf.variable_scope('interaction_layer'):
            ...
            # shape of [None,1]
            self.interacton_terms = tf.multiply(0, 5, tf.reduce_mean(tf.subtract(tf.pow(tf.sparse_tensor_dense_matmul(self.X, v), 2),
                                                                                 tf.sparse_tensor_dense_matmul(self.X, tf.pow(v, 2))),
                                                                     1, keep_dims=True))
            # shape of [None,2]
            self.y_out = tf.add(self.linear_terms, self.interacton_terms)
