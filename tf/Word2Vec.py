...
# step3、生产skip-gram model的训练batch##############################
data_index = 0
...
with graph.as_default():
    ...
    with tf.device('/gpu:0'):
        # 初始化词向量：随机生成所有单词的词向量embeddings,(weigth初始化)
    ...
    # 定义损失
    loss = tf.reduce_mean(tf.nn.nce_loss(weights=nce_weigths, biases=nce_biases, labels=train_labels, inputs=embed,
                                         num_sampled=num_sampled, num_classes=vocabulary_size))

    # 定义优化方法
    optimizer = tf.train.GradientDescentOptimizer(1.0).minimize(loss)
    # 占比概率形式，归一化norm算embedding的模大小
...
with tf.Session(graph=graph, config=tf.ConfigProto(allow_soft_placement=True,
    ...
    for step in range(num_steps):
        ...
        # 每10000词，计算一次验证单词与全部单词的相似度，并将与每个验证单词最相似的8个单词展示出来
        if step % 10000 == 0:
            sim = similarity.eval()
            print(sim)
