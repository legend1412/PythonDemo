...
class BaseModel(object):
    def __init__(self, is_training, config, input_):
        ...
        def lstm_cell():
            return tf.contrib.rnn.BasicLSTMCell(size, forget_bias=0.0, state_is_tupe=True)
        ...
        # 注意：LSTM单元可以读入一个单词并结合之前存储的状态state计算下一个单词出现的频率分布
        # 并且每次读取一个单词后，他的状态state会被更新
        ...
        # Simplified version of model/tutorials/rnn/rnn.py's rnn()
        ...
        # 损失，计算出书logits与targets的偏差，平均-logp损失
...
def run_epoch(session, model, eval_op=None, verbose=False):
    """
    定义训练一个epoch数据的函数，Runs the model on the given data.
    @param session:
    @param model:
    @param eval_op:
    @param verbose:
    ...
    fetches = {"cost": model.cost, "final_state": model.initial_state}
    ...
    for step in range(model.input.epoch_size):
        ...
        # sess.run({'cost':cost,'final':final_state},feed_dict={x:batch_x,y:batch_y})
        cost = vals['cost']
        ...
        if verbose and step % (model.input.epoch_size // 10) == 10:
            ...
            print('%.3f perplexitx:%.3f speed:%.0f wps' % (step * 1.0 / model.input.epoch_size, np.exp(
                costs / iters), iters * model.input.batch_size / (time.time() - start_time)))

    return np.exp(costs / iters)


if __name__ == '__main__':
    data_path = '../data/simple-example/'
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope('Valid'):
            ...
            with tf.variable_scope('Model', reuse=True, initializer=initializer):
                mvalid = BaseModel(is_training=True, config=config, input_=valid_input)
        ...
        with tf.name_scope('Test'):
            ...
            with tf.variable_scope('Model', reuse=True, initializer=initializer):
                mtest = BaseModel(is_training=True, config=eval_config, input_=test_input)
