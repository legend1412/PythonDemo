...
def user_item_score(action_sum=100, tag='sum'):
    """
    :param 将原始数据处理成cf的输入数据，类似udata中的数据 user_id,item_id,rating
    :return data（DataFrame）[user_id,item_id,score]
    """
    # 获取用户行为数据
    df_user_watch = conf.user_watch(action_sum)

    #  思考怎么获取到score：stay_second/音乐的total_timelen，听这个音乐的占比
    #  stay_seconds在user_watch，total_timelen在music_meta
    df_music_meta = conf.music_data()
    #  pandas里的merge相当于hive里面的join
    data = df_user_watch.merge(df_music_meta, how='inner', on='item_id')
    # 从内存中删除基础数据表
    del df_user_watch
    del df_music_meta
    # pandas apply 相当于spark rdd map 操作，score=20s/300s  pandas官网简单实用说明 spark官网
...
def train_from_df(df, col_name=None):
    """
    ...
    if col_name is None:
        col_name = ['user_id', 'item_id', 'score']
    d = dict()
...
if __name__ == '__main__':
    # 控制用户数据量，在计算用户与用户相似度
    testdata = user_item_score(10000)
    train = train_from_df(testdata)
    del testdata
    # 将训练数据存储起来，这样下次使用直接读取，不需要再处理
