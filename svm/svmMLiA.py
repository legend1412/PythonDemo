...
def smo_simple(data_mat_in, class_labels, C, toler, max_iter):
    ...
    while iter < max_iter:
        ...
        # 扫训练集
        for i in range(m):
            # 模型的函数，已经对w，b求导了，是将w带入模型函数中的fxi = sum(ai*yi*xi^T*x+b)
            ...
            if ((label_mat[i] * Ei < - toler) and (alphas[i] < C)) or ((label_mat[i] * Ei > toler) and (alphas[i] > 0)):
                ...
                if L == H:
                    print('L==H')
                    continue
                # eta 统计学习方法p127-p128 推导过程 公式-eta
                ...
                # 综上都在更新aj，当aj不能再更新的时候，更新ai。-yi*yi（ai_new-aj_old）
                alphas[i] += label_mat[j] * label_mat[i] * (alpha_j_old - alphas)  # update i by the same amout as j
                # 将样本（xi，yi）带入模型函数得到的
...
class Optstruct:
    def __init__(self, data_mat_in, class_labels, C, toler, kTup):  # Initialize the structure with the parameters
        ...
        for i in range(self.m):
            self.k[:, 1] = kernel_trans(self.X, self.x[i, :], kTup)
...
def select_j(i, oS, Ei):  # this is the second choice - heurstic ,and calcs Ej
    ...
    max_delta_e = 0
    Ej = 0
    oS.eCache = [1, Ei]  # set valid # choose the alpha that gives the maximum delta E
...
def innerL(i, o_s):
    Ei = calc_ek(o_s, i)
    if ((o_s.labelMat[i] * Ei < -o_s.tol) and (o_s.alphas < o_s.C)) or ((o_s.labelMat[i] * Ei > o_s.tol) and (o_s.alphas[i] > 0)):
        j, Ej = select_j(i, o_s, Ei)
        alpha_i_old = o_s.alphas[i].copy()
        alpha_j_old = o_s.alphas[j].copy()
        if o_s.labelMat[i] != o_s.labelMat[j]:
            L = max(0, o_s.labelMat[j] - o_s.labelMat[i])
        ...
        if abs(o_s.alphas[j] - alpha_j_old) < 0.00001:
            print('j not moving enough')
            return 0
        o_s.alphas[i] += o_s.labelMat[j] * o_s.labelMat[i] * (alpha_j_old - o_s.alphas[j])  # update i by the same amount as j
        updateEk(o_s, i)  # added this for the Ecache #the update is in the oppostie direction
        b1 = o_s.b - Ei - o_s.labelMat[i] * (o_s.alphas[i] - alpha_i_old) * o_s.K[i, i] - o_s.labelMat[j] * (o_s.alphas[j] - alpha_j_old) * \
             o_s.K[i, j]
        b2 = o_s.b - Ej - o_s.labelMat[i] * (o_s.alphas[i] - alpha_i_old) * o_s.K[i, j] - o_s.labelMat[j] * (o_s.alphas[j] - alpha_j_old) * \
             o_s.K[j, j]
    ...
    while (iter < max_iter) and ((alpha_pairs_changed > 0) or (entire_set)):
        ...
        if entire_set:  # go over all
            for i in range(o_s.m):
                ...
                print('fullSet,iter:%d i:%d,paris chenged %d ' % (iter, i, alpha_pairs_changed))
            iter += 1
...
def testDigits(k_tup=('rbf', 10)):
    ...
    error_count = 0
    for i in range(m):
        kerner_eval = kernel_trans(sVs, dat_mat[i, :], k_tup)
        predict = kerner_eval.T * multiply(labelSV, alphas[sv_ind]) + b
        if sign(predict) != sign(label_arr[i]):
            error_count += 1
    print('the training error rate is: %f' % (float(error_count) / m))
